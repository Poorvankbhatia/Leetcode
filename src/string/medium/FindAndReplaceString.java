...
public class FindAndReplaceString {
    ...
    public String findReplaceString(String S, int[] indexes, String[] sources, String[] targets) {
        Map<Integer, String> indexSources = new HashMap<>();
        Map<Integer, String> indexTargets = new HashMap<>();

        for (int i = 0; i < indexes.length; ++i) {
            indexSources.put(indexes[i], sources[i]);
            indexTargets.put(indexes[i], targets[i]);
        }
        StringBuilder sb = new StringBuilder();

        for (int i = 0; i < S.length(); ++i) {
            if (indexSources.containsKey(i) && S.substring(i).startsWith(indexSources.get(i))) {
                sb.append(indexTargets.get(i));
                i += indexSources.get(i).length() - 1;
            }
            else {
