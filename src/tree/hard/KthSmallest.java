...
public class KthSmallest {
    ...
    public static void main(String[] args) {
        System.out.println(new KthSmallest().findKthNumber(428934284,11922239));
    }
...
we don't really need to do a exact k steps preorder traverse of the denary tree, the idea is to calculate the steps between curr and curr + 1
...
else if the steps > k, that means the curr + 1 is actually behind the target node x in the preorder path, we can't jump to curr + 1. What we
have to do is to move forward only 1 step (curr * 10 is always next preorder node) and repeat the iteration.
