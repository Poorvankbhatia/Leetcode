...
Explanation:
We only have one website domain: "discuss.leetcode.com". As discussed above, the subdomain "leetcode.com" and "com" will also be
...
public class SubdomainVisitCount {
    ...
    public List<String> subdomainVisits(String[] cpdomains) {
        ...
        for (String cpdomain : cpdomains) {
           ...
           if(!map.containsKey(fullDomain)) {
               map.put(fullDomain,c);
           } else {
               map.put(fullDomain,map.get(fullDomain)+c);
           }

            for (int i=0;i<fullDomain.length();i++) {
                if(fullDomain.charAt(i)=='.') {
                    String s = fullDomain.substring(i+1);
    ...
    public static void main(String[] args) {
        String[] domains = new String[]{"654 yaw.lmm.ca", "1225 lmm.ca"};
