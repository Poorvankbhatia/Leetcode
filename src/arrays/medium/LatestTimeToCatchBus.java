...
minutes if you arrive at y minutes where y <= x, and the bus is not full. Passengers with the earliest arrival times get on the bus first.
...
public class LatestTimeToCatchBus {
    ...
    public int latestTimeCatchTheBus(int[] buses, int[] passengers, int capacity) {
        ...
        for (int bus : buses) {
            int count = 0;
            while (j < passengers.length && passengers[j] <= bus && count < capacity) {
    ...
    public static void main(String[] args) {
        System.out.println(new LatestTimeToCatchBus().latestTimeCatchTheBus(new int[]{20,30,10},
