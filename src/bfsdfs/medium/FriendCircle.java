...
public class FriendCircle {
    ...
    public int findCircleNum(int[][] M) {
        ...
        int row = M.length; // no of students
    ...
    private void circleCount(int[][] M,boolean[] visited,int currentRow) {

        visited[currentRow] = true;

        for (int i=0;i<M[currentRow].length;i++) {
            if(M[currentRow][i]==1 && i!=currentRow && !visited[i]) {
                circleCount(M,visited,i);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.print(new FriendCircle().findCircleNum(arr));
