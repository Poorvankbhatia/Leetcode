...
public class FoodRatings {
    ...
    private class Item {
        ...
        public Item(String food, int rating) {
            this.food = food;
            this.rating = rating;
        }
    }

    Map<String, TreeSet<Item>> cuisineMap;
    ...
    public FoodRatings(String[] foods, String[] cuisines, int[] ratings) {
        ...
        for(int i=0;i<foods.length;i++) {
            ...
            cuisineMap.putIfAbsent(cuisines[i],new TreeSet<>((a,b)->(b.rating!=a.rating?b.rating-a.rating:a.food.compareTo(b.food))));
            cuisineMap.get(cuisines[i]).add(new Item(foods[i],ratings[i]));
        }
    }

    public void changeRating(String food, int newRating) {
        Item item = new Item(food, ratingMap.get(food));
        TreeSet<Item> items = cuisineMap.get(foodToCuisineMap.get(food));
        ratingMap.put(food, newRating);
        items.remove(item);
        items.add(new Item(food, newRating));
    }
